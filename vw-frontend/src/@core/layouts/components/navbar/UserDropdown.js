  ...
  useEffect(() => {
    if (isUserLoggedIn() !== null) {
      if (window.location.href.indexOf("/admin/") > -1) {
        ...
        setUserData(JSON.parse(localStorage.getItem("userDataAdmin")));
      } else if (window.location.href.indexOf("/gas-station/") > -1) {
        setUserData(JSON.parse(localStorage.getItem("userDataGasStation")));
      } else if (window.location.href.indexOf("/gas-station-network/") > -1) {
        setUserData(
          JSON.parse(localStorage.getItem("userDataGasStationNetwork"))
        );
      } else if (window.location.href.indexOf("/supervisor/") > -1) {
        setUserData(JSON.parse(localStorage.getItem("userDataSupervisor")));
      } else {
        setUserData(JSON.parse(localStorage.getItem("userDataCustomer")));
      }
    }
  }, []);
