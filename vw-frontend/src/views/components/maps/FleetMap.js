...
class FleetMap extends React.PureComponent {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            position: null,
            rowData: null,
            customRoute: this.props.customRoute,
    ...
    componentDidMount() {
        if (this.props.mapmonitoringdata) {
            this.setState({
                data: this.props.mapmonitoringdata
            })
        }
    }
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.state.data != this.props.mapmonitoringdata) {
            this.setState({
                data: this.props.mapmonitoringdata
            })
        }
                    ...
                    this.state.customRoute ?
                        this.props.data.map((marker, index) => (
                            <Marker
                                ...
                                icon={{
                                    url: marker.icon ? jwtDefaultConfig.BASE_IMAGE_URL + marker.icon : `/images/markers/car.png`,
...
const mapStateToProps = (state) => {
    return {
        mapmonitoringdata: state.monitoringMapData.data
    };
};
const wrap = connect(mapStateToProps)(FleetMap)
