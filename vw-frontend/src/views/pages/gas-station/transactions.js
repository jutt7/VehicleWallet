  ...
  const getData = () => {
    ...
    axios
      .get(
        ...
        }/fuel_requests?page=${currentPage}&from=${from}&to=${to}&transaction_status=${transaction_status}&gas_station_id=${
          props.stationID
            ? props.stationID
            : helper.getIDfromUrl(window.location.href)
        }&sub_client_id=${selctClient}`
      )
      .then((res) => {
        helper.redirectToLogin(helper.isObject(res.data) ? res.data.code : 200);
        if (res.status && res.status === 200) {
      ...
      <Card>
        <CardHeader className="border-bottom">
          <CardTitle style={{ fontWeight: "bold" }} tag="h4">
            {t("Gas Station Transaction")} {props.gasStationName}
          </CardTitle>
        </CardHeader>
                ...
                {data &&
                  data.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>{item ? getStatusLabels(item) : ""}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
