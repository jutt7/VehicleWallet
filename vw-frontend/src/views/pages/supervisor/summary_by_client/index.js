...
import TotalFuel from "./totalFuel";
...
export default function client(props) {
  ...
  const onOpenDeleteModal = (rowData, index) => {
    ...
    setonSubmit("delete");
    setdeleteMessage(rowData.status === 1 ? "block" : "activate");
  ...
  const getData = () => {
    if (!helper.isObject(selectedClient)) {
      helper.toastNotification(
        "Please select client to fetch records.",
        "FAILED_MESSAGE"
      );
      return false;
    }
    ...
    axios
      .post(
        `${jwtDefaultConfig.adminBaseUrl}/client-summary?page=${currentPage}&pagination=true`,
        {
          interval: {
      ...
      .then((res) => {
        ...
        if (res.status && res.status === 200) {
          ...
          setStats({
            ...
            fuel_in_liters: res.data.fuel_in_liters,
            number_of_transactions: res.data.total_transactions,
            client_name: helper.shortTextWithDots(selectedClient.label, 15),
          });
  ...
  const getClients = () => {
    axios
      .get(`${jwtDefaultConfig.adminBaseUrl}/all_clients?web=true&lang=en`)
      .then(async (res) => {
        helper.redirectToLogin(helper.isObject(res.data) ? res.data.code : 200);
        if (res.status && res.status === 200) {
  ...
  return (
    <div>
      ...
      {data.length ? (
        <Row>
          <Col>
            <TotalAmount kFormatter={kFormatter} Count={stats.amount} />
          </Col>
          <Col>
            <TotalFuel kFormatter={kFormatter} Count={stats.fuel_in_liters} />
          </Col>
          <Col>
            <ClientName kFormatter={kFormatter} Count={stats.client_name} />
          </Col>
          <Col>
            <TotalTransactions
              kFormatter={kFormatter}
              Count={stats.number_of_transactions}
            />
          </Col>
        </Row>
      ) : (
