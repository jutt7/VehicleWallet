...
import HeldTransactionsCount from "./heldTransactionsCount";
import HeldTransactionsAmount from "./heldTransactionsAmount";
...
export default function client(props) {
  ...
  const onOpenCommentModal = (item, index) => {
  ...
  const getData = () => {
    ...
    axios
      .post(
        `${jwtDefaultConfig.adminBaseUrl}/held-transactions?page=${currentPage}&pagination=true`,
        {
          interval: {
      ...
      .then((res) => {
        ...
        if (res.status && res.status === 200) {
          ...
          setheld_transaction_count(res.data.count_transaction_amount);
          setheld_transaction_amount(
            helper.applyCountID(res.data.held_transaction_amount)
          );
  ...
  const create = (args) => {
    setoverlay(true);
    axios
      .post(`${jwtDefaultConfig.adminBaseUrl}/store-comments`, {
        comment: {
  ...
  return (
    <div>
      ...
      <Row>
        <Col>
          <HeldTransactionsCount
            kFormatter={kFormatter}
            heldTransaction={held_transaction_count}
          />
        </Col>
        <Col>
          <HeldTransactionsAmount
            kFormatter={kFormatter}
            heldTransactionAmount={held_transaction_amount}
          />
        </Col>
      </Row>
        ...
        <Col lg={12}>
          <Row>
            ...
            <Col>
              ...
              <input
                onChange={(e) => {
                  onChangeInput(e.target.value, "from");
                }}
                value={filter.from}
                className="form-control"
                name="from"
              ...
              data.map((item, index) => (
                ...
                >
                  ...
                  <td>
                    <Button
                      color="success"
                      onClick={(e) => onOpenCommentModal(item, index)}
                    >
                      Add Comment
                    </Button>
                  </td>
