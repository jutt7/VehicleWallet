...
const ApexBarChart1 = ({ primary, direction, transactionPicker }) => {
  ...
  const getData = () => {
    if (transactionPicker.length == 2) {
      setoverlay(true);
      axios
        .post(`${jwtDefaultConfig.adminBaseUrl}/vehicle-graph`, {
          graph: getDetail(),
        })
        .then((res) => {
          ...
          if (res.status && res.status == 200) {
            // console.log("charts data -------------", res.data.data);
